...
function get-dnszone {
    ...
    #Set Secret Server fields for data retrieval
    $username = "CloudflareSSAPI"
    $uri = "https://secretserver.dhpace.com/SecretServer/oauth2/token"
    $body = @{
        grant_type = 'password'
        username   = $username
        password   = "$env:cloudflareapipass"
    }

    Write-Host "Grabbing Secret Server access token..."
    #Grab secret server access token
    $response = invoke-restmethod -method post -uri $uri -body $body -headers $null
    $token = $response | select -expand access_token

    $header = @{ "Authorization" = "Bearer $token" }
    $pwsurl = "https://secretserver.dhpace.com/SecretServer/api/v2/secrets/5206"
    Write-Host "Requesting dnsimple account info from Secret Server..."
...
Credentials are accessed using Secret Server API calls, as well as locally-stored environment variables in order to mask username fields.
#>
...
|___|___|  /__||__| |__(____  /____/__/_____ \__|___|  /\___  /   \______  /|__|    \___  >____ /____  >
    ...
    $header = @{ "Authorization" = "Bearer $token" }
    $pwsurl = "https://secretserver.dhpace.com/SecretServer/api/v2/secrets/5638"
    $pwsurl2 = "https://secretserver.dhpace.com/SecretServer/api/v2/secrets/5666"
    Write-Host "Requesting cloudflare account info from Secret Server, then CSC info..."
 ...
 \______  /___|  (____  /___|  /__|___|  /\___  /   \______  /_______  / \______  /  |____|_  /\___  >\___  >____/|__|  \____ /____  >
    ...
    $body = [ordered]@{
        ...
        "notifications" = [ordered]@{
            "enabled" = "true"
            "additionalNotificationEmails" = @("Blake.Miller@dhpace.com","Matt.Melone@dhpace.com")
        }
        "showPrice" = "true"
        "customFields" = @()
    }
    ...
    if ($j -ne -1) {
        ...
        if (Test-Path "C:\Users\$env:username\Downloads\dnsimple\$domain'URI'.txt") {
            ...
            $redirect = foreach ($src in $content) {
                ...
                if ($? -eq $true) {
                    [PSCustomObject]@{
                        #Set source anb destination URL into their own objects.
            ...
            #If there are more than 0 URL forwards.
            if ($redirect.Count -gt 0) {
                $c = 0
                $destNum = @()
                foreach ($number in $redirect.dest) {
                    #Add each destination url to the array for later.
                    $destNum += $redirect.dest[$c]
                    $c++
                }
            }
            ...
            if ($testSet.result.rules.count -lt $redirect.Count -and $testSet.result.rules.Count -gt 0) {
            ...
            if ($testSet.result.rules.Count -eq 0) {
                    Write-Host "No rules found in this ruleset. We will create a rule for"$redirect.Count" URI record(s)."
                $r = 0
